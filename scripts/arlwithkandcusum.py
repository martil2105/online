...
rhos = 0
#k = 5
thresholds = [0.6,0.7,0.8,0.85,0.9,0.98]
k_values = [1,2,3]
#load model
...
for i in range(len(k_values)):
    plt.plot(thresholds, detection_delay[:,i], label=f"K={k_values[i]}",marker="o")
...
plt.subplot(1, 3, 2)
for i in range(len(k_values)):
    plt.plot(thresholds, average_run_length[:,i], label=f"K={k_values[i]}",marker="o")
plt.xlabel("Threshold")
...
plt.subplot(1, 3, 3)
for i in range(len(k_values)):
    plt.plot(thresholds, false_positive_rate[:,i], label=f"K={k_values[i]}",marker="o")
plt.xlabel("Threshold")
