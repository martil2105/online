...
window_length = 100
num_repeat = 5
stream_length = 500 #previous 20000
sigma = 1
seed_val = [2023]
...
for iter in range(max_iter):
    ...
    for i in range(num_streams):
        stream = data_alt[i]
        det_time, _ = detect_change_in_stream_cusum(stream, model, window_length, threshold)

        if det_time == 0:                           
            false_negatives += 1
        elif det_time <= true_tau_alt[i]:           
            false_positives += 1
    ...
    if current_edd < target_edd or not fp_ok:
        threshold_low  = threshold      #more sensitive
    else:
        threshold_high = threshold      #less sensitive
else:
    print("Reached max.")

final_threshold = threshold

print(f"threshold={threshold}, EDD={current_edd:}, "f"FP={false_positives}, FN={false_negatives}")
