...
def detect_change_in_stream_loc(stream, model, window_length, threshold):
    ...
    return detection_time , np.max(probabilities)

# Generate null data and threshold percentile
data_null = GenDataMean(num_repeats, stream_length, cp=None, mu=(mu_L, mu_L), sigma=sigma)
num_streams = data_null.shape[0]
max_probabilities = []
for i in range(num_streams):
    dt, max_prob = detect_change_in_stream_loc(data_null[i], model, window_length, thresholds[0])
...
for idx, percentile in enumerate(percentiles):
    for i in range(num_streams):
        dt, _ = detect_change_in_stream_loc(data[i], model, window_length, percentile)
        if dt > 0:
            arl[idx,i] = dt
        else:
...
for idx, percentile in enumerate(percentiles):
    for i in range(num_repeats):
        dt, _ = detect_change_in_stream_loc(data_alt[i], model, window_length, percentile)
