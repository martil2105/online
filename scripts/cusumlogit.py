...
iter = 0
file_path = Path(script_dir, "optimal_threshold_cusum_forarl02000.npy")
threshold = np.load(file_path)
...
for i in range(num_streams):
    ...
    if detection_time == 0:
        ...
        if detection_time > true_tau_alt[i]:
            delay.append(detection_time-true_tau_alt[i])
        else:
            false_positives += 1
