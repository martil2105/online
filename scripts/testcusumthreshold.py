...
for stream_length in stream_lengths:
    ...
    for i in range(num_streams):
        ...
        detection_time, cusum_scores = detect_change_in_stream_batched_cusum(stream_null, model, window_length, threshold_final)
        ...
        detection_time, _ = detect_change_in_stream_batched_cusum(stream_alt, model, window_length, threshold_final)
...
plt.subplot(1, 2, 1)
plt.plot(stream_lengths, arls, "o-", label="ARL₀")
plt.xlabel("Stream length")
plt.ylabel("ARL₀")
plt.title("ARL₀ vs Stream length")
plt.legend()
plt.subplot(1, 2, 2)
plt.plot(stream_lengths, dd, "o-", label="Detection Delay")
plt.xlabel("Stream length")
